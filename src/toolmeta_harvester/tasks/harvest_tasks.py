...
def get_tools_from_db():
    ...
    with Session(engine) as session:
        tools = session.query(Tool.id, Tool.name, Tool.version).all()
        return tools
...
def get_error_repositories():
    ...
    with Session(engine) as session:
        error_repos = session.query(Repository).filter_by(status="error").all()
        return [r.url for r in error_repos]
        # for repo in error_repos:
        #
        #     print(f"Error repository: {repo.url} with error code: {repo.eror_code}")

def get_all_repositories():

    """Process repositories with error status."""
    with Session(engine) as session:
        error_repos = session.query(Repository).all()
        for repo in error_repos:
...
def process_single_repository(repo_url, session):
        ...
        for url in tool_folders:
            results = session.query(Repository).filter_by(url=url).all()
            repo = None
            if len(results) > 0:
                repo = results[0]
                if repo.status == "processed":
                    print(f"Repository {url} already exists in the database. Skipping.")
                    continue
            else:
                repo = Repository(
                    url=url,
            ...
            try:
                ...
                for tool in tools:
                    try:
                        ...
                        print(f"Adding tool: {db_tool.id}, {db_tool.name}, version: {db_tool.version}")
